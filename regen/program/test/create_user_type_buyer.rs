...
mod create_user_type_buyer {
	...
	fn test_transaction_is_ok() {
		...
		let shipping_label: String = Default::default();
		let company_seed_user: Pubkey = Pubkey::new_unique();
		let shipping_seed_company: Pubkey = Pubkey::new_unique();
		...
		let (shipping_pubkey, _) = Pubkey::find_program_address(
		...
		let accounts = vec![
			...
			AccountMeta::new(shipping_pubkey, false),
			AccountMeta::new_readonly(system_program::id(), false),
		];
		let data = RegenInstruction::CreateUserTypeBuyer(crate::generated::instructions::CreateUserTypeBuyerArgs{
			user_id,
			company_name,
			registration_number,
			address,
