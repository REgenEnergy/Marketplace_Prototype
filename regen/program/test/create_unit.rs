...
mod create_unit {
	...
	fn test_transaction_is_ok() {
		...
		let conversion_rate: u64 = Default::default();

		let (unit_pubkey, _) = Pubkey::find_program_address(
			&[b"auction_weight_unit", name.as_bytes().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(unit_pubkey, false),
			AccountMeta::new_readonly(system_program::id(), false),
		];
		let data = RegenInstruction::CreateUnit(crate::generated::instructions::CreateUnitArgs{
			name,
			conversion_rate,
		});
		let signers = vec![
